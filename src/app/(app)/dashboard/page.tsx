...
function ScoreRing({ score, size = 64 }: { score: number; size?: number }) {
  ...
  const color = score >= 75 ? "#30D158" : score >= 50 ? "#0A84FF" : "#FF453A";
  ...
  return (
    <div className="relative inline-flex items-center justify-center" style={{ width: size, height: size }}>
      ...
      <span className="absolute font-mono text-[14px] font-medium text-white">{score}</span>
    </div>
  );
}
      ...
      <div className="relative z-10 space-y-12">
            ...
            {QUICK_ACTIONS.map((action, i) => {
              ...
              return (
                <Link key={action.href} href={action.href}>
                  <motion.div
                    whileHover={{ y: -2 }}
                    className="glass rounded-xl border border-[rgba(255,255,255,0.06)] bg-[rgba(22,22,23,0.3)] hover:bg-[rgba(255,255,255,0.02)] hover:border-[rgba(255,255,255,0.15)] p-5 flex items-center gap-4 cursor-pointer group transition-all duration-300"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="text-white font-medium text-[14px] group-hover:text-blue transition-colors duration-200">{action.label}</span>
                    ...
                    <ChevronRight className="w-4 h-4 text-stone-ghost group-hover:text-white group-hover:translate-x-0.5 transition-all flex-shrink-0" />
                  </motion.div>
                </Link>
              );
            })}
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <Link 
              href="/history" 
              className="text-blue text-[12px] hover:underline transition-colors flex items-center gap-1 font-medium"
            >
              History Log <ChevronRight className="w-3 h-3" />
            </Link>
          </div>
          
          <div className="glass rounded-xl border border-[rgba(255,255,255,0.06)] bg-[rgba(22,22,23,0.1)] overflow-hidden divide-y divide-[rgba(255,255,255,0.04)]">
              ...
              <Link key={debate.id} href={`/debate/session?id=${debate.id}&topic=${encodeURIComponent(debate.topic)}&mode=socratic`}>
                <div className="p-4 flex items-center justify-between hover:bg-[rgba(255,255,255,0.01)] cursor-pointer group transition-colors duration-150">
                  <div className="flex-1 min-w-0 pr-4">
                    <p className="text-white text-[14px] font-medium truncate group-hover:text-blue transition-colors duration-150">
                      {debate.topic}
                    </p>
                    <div className="flex items-center gap-3 mt-1.5">
                      ...
                      {debate.fallacies > 0 && (
                        <>
                          <span className="text-[11px] text-stone-ghost">•</span>
                          <span className="text-[11px] text-[#FF453A] flex items-center gap-1">
                  ...
                  <div className="flex items-center gap-4 flex-shrink-0">
                    <div className="text-right">
                      <span 
                        ...
                        style={{ color: debate.score >= 75 ? "#30D158" : debate.score >= 60 ? "#0A84FF" : "#FF453A" }}
                      >
                        {debate.score}%
                      </span>
                      <p className="text-[10px] text-stone-ghost uppercase tracking-wide">Rigor</p>
                    </div>
                    <ChevronRight className="w-4 h-4 text-stone-ghost group-hover:text-white group-hover:translate-x-0.5 transition-all" />
                  </div>
                </div>
              </Link>
            ))}
